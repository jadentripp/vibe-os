...
`audio-proof.json`, writes `human-playtest-notes.txt`, writes a
...
the expected status, ELF, symbol, log, optional aggregate-audio, and notes files
with byte counts and SHA-256 hashes, plus the status-only artifact policy. The
artifact checker requires this manifest in `--human-session` mode and rejects
