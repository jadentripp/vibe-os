...
handoff report `argvsrc=2`, and that `pmask` plus
`pkind`/`peip`/`pcr3`/`pkstk` show IRQ switches in both directions between Doom
and the preempt probe with distinct address spaces and kernel stacks. It is a
cloud artifact checker, not a claim that the running
