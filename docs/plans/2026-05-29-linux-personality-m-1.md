...
**Architecture:** Add a per-process **personality flag** to the existing process record. The `int 0x80` dispatcher (currently a `cmp eax, SYS_*` chain at `kernel/kernel.asm:22429`) branches to a **separate Linux i386 syscall path** when the flag is `LINUX`; native programs (Doom/Quake) are untouched. A new **Linux ELF32 loader** maps `PT_LOAD` segments and builds a SysV-i386 initial stack (argc/argv/envp + minimal auxv). The current proof kernel keeps a compact `linux_last_unimpl_nr` status field rather than a full trace ring, and the BIOS boot path now reads a 360-sector kernel window (184320 bytes) with the temporary ELF load buffer at `0x72000`; the smoke status window is 32 KiB at `0x77000`, and the PMM frame map lives at `0x50000` so low kernel stacks/page tables and frame accounting stay separated.
...
Current Chromium pressure proof: the current available Debian sid i386 `chromium_148.0.7778.178-1_i386.deb` binary package is packaged as `/BIN/CHROMIUM.ELF` from `/usr/lib/chromium/chromium`. The package SHA-256 is `627276adfbd983e1537403f9f8382db27f100f3944df1f2279ca3cc65b93665e`; the browser ELF SHA-256 is `8a65db2dac7bebd1cf932290139e110fae970efbf6be5a2351188f824fee7c2a`. Matching `chromium-common_148.0.7778.178-1_i386.deb` supplies `/BIN/ICUDTL.DAT`; its package SHA-256 is `26344d8b9cf82e61ad67240f46ccf24cf2b902a47d29e51681c3440842c735b3`, and `icudtl.dat` SHA-256 is `bd8c145abdf3f8383276ce01dfa4ae48709bef9fef1c0711eb7c3fab4f6eb7c2`. The current Chromium smoke uses separate ignored Debian sid i386 `libc6_2.42-16_i386.deb` artifacts for `/LIB/LDLINUX.SO2` and `/LIB/LIBC.SO6`; the package SHA-256 is `4581e89be4256fe0b75c1ba7090477b65d1a4c893ddc0d438c518c6dd8e8e8fb`, ld-linux SHA-256 is `2527ad9fdf19bc90734f17e10922d188dff9095abfcc1d06f97a0c1443f9f508`, and libc SHA-256 is `7361db7f1c6692adedfb8a1cc4956cb0bddef201aef3aa4ba53a28f0ceff8c51`. After relocating the smoke-status window away from live CPU tables, the latest focused smoke stages the 101 recursive real sid i386 `NEEDED` shared-library assets. The parent QEMU run after argv/status instrumentation reports `exec=OK path=/BIN/CHROMIUM.ELF argc=0000000B largedem=00000002/00000002/00000000/00000002/08010D76/08010000/00010000/00001000 demstatus=00000002/00000002/00000002/00000000/0000000E/00000000/00000000 dseq=00000002/00000002/00000000/00000003/000000DB/00000127/00EA7B9C pexit=FFFFFFFF/00000000/00000000/00000000/00000000 pfault=FFFFFFFF/00000000/00000000/00000004/0000000E/08010D76 faultres=00000000/00000000/00000000/00000000/00000000/00000001 panic=NONE shutdown=NONE pmmchk=OK`, with Linux personality selected and real sid ld-linux loaded. The proof path now honestly covers the browser ELF, staged argv, `PT_INTERP`, ET_DYN load bias, initial sparse mapping, `PT_DYNAMIC` premap, ld.so staging reservation, and early file-backed demand paging. The code also contains synthetic Linux library dirs, optional `rseq(386) -> -ENOSYS`, SSE/FPU context save/restore, seed `socketpair`, `setsockopt(SOL_SOCKET, SO_PASSCRED)`, and fork-like `clone` support, but the latest relocated-status proof has not re-reached socket/clone yet. Browser UI remains unproved; the fresh boundary is early ld.so/Chromium demand paging around `/etc/ld.so.cache -> -ENOENT` and a user page fault at `0x08010D76`, not the older mid-instruction #UD.

Current packaging correction: the separate exit-`127` wrapper run came from bad hand-staging of a no-version `libnspr4.so` alias. The helper-generated asset set maps the package's real `libnspr4.so` soname to the correct `/LIB/NSPR4.SO` alias family, so the current proof boundary remains the relocated early demand-page path around `/etc/ld.so.cache -> -ENOENT` and `0x08010D76`; socket/clone/#UD observations still need fresh revalidation after that boundary is pushed again.
...
**Next:** continue M1a from the static BusyBox echo/true proof toward BusyBox `ash`, `ls`, `cat`, and scripted shell behavior; in parallel, harden the M0 dynamic-linker seed around real signal/futex semantics and protection behavior. For the browser pressure path, push the current early demand-page boundary, then re-reach the socket/clone surface and fill the next real Linux surface (`/usr/lib/os-release`, `/proc`, `/dev`, broader sockets, resource packs/locales, graphics/input, process/sandbox).
