...
- `tools/build_busybox_vibe.sh` / `build/busybox-i386/busybox-vibe` — shell-only BusyBox 1.35.0 static i386 musl builder and ignored `ET_EXEC` artifact (currently 166960 bytes, linked at `0x00e80000`) installed by `tools/mkrootfs.sh` as `/BIN/BUSYBOX.ELF` when present.
...
Current M1a BusyBox seed: `tools/build_busybox_vibe.sh` is a shell-only/no-network builder that uses the existing ignored BusyBox source tree plus Zig target tools. It builds BusyBox 1.35.0 as a static i386 musl `ET_EXEC` binary, no PIE, linked at `0x00e80000`, copied to `build/busybox-i386/busybox-vibe`, and installed by `tools/mkrootfs.sh` as `/BIN/BUSYBOX.ELF` when present. The current artifact is 166960 bytes. `tools/mkrootfs.sh` also installs `/ETC/CATOK.TXT` from `tests/linux/catok.txt` when present; set `VIBE_SKIP_BUSYBOX_BUILD=1` to reuse an existing BusyBox artifact. The built applets are `ash`, `sh`, `echo`, `true`, `ls`, `cat`, `cp`, `grep`, `sleep`, and `ps`. Focused QEMU smokes boot with `LINUX_M1_SMOKE`, exec `/BIN/BUSYBOX.ELF` through the Linux personality, and stage argv as real BusyBox applet invocations:
...
- `busybox sh -c 'echo busybox-ok'`, `busybox ls /BIN`, `busybox cat /ETC/CATOK.TXT`, `busybox cp /ETC/CATOK.TXT /CP.TXT`, `busybox grep ... /ETC/CATOK.TXT`, `busybox sleep 1`, and focused `busybox ps` now exit `0` cleanly; the cp proof also asserts `/CP.TXT` appears in the post-run FAT image with the expected size.

The successful BusyBox applet proofs require/tolerate the existing static-libc startup surface plus `prctl(172)` as a no-op and `sched_getaffinity(242)` returning a single-CPU mask. `ash`/`sh` are compiled in and the non-interactive `sh -c` selector is now clean, but it is not the M1 interactive shell yet. The focused `ps` selector proves a minimal synthetic `/proc` enough for the applet, not the full procfs/devfs/symlink surface. Root `ls /` still has a special-entry stat issue. Full M1 remains open until an interactive prompt, cwd/dirfd behavior, pipes/redirection, Ctrl-C job/signal behavior, generic procfs/devfs/symlink surfaces, and broader coreutils pass.
