...
> **Current branch status:** Tasks 1-3 have landed on `feat/linux-personality-m-1`: `edda888` adds the Linux `-ENOSYS` default proof state, `bb84f67` adds the per-process personality flag and dispatch branch, and `f8cd8a0` adds the `hello_write` test-binary/rootfs scaffold. The current worktree extends that scaffold with `auxv_dump`, `tls_probe`, `startup_probe`, `exec_limits_probe`, `dir_probe`, `cwd_dirfd_probe`, `fd_probe`, `pipe_probe`, `fork_probe`, and a Zig-built static musl `hello_musl` acceptance binary. Flagged QEMU runs have proved `/BIN/HELLO.ELF`, `/BIN/AUXV.ELF`, `/BIN/TLS.ELF`, `/BIN/STARTUP.ELF`, `/BIN/XLIMIT.ELF`, `/BIN/DIR.ELF`, `/BIN/CWDDIRFD.ELF` (`cwd dirfd ok`, including negative relative-miss checks after `chdir("/BIN")`, exit `41`), `/BIN/FD.ELF`, `/BIN/PIPE.ELF`, `/BIN/FORK.ELF` (`fork ok`, full-copy `fork`/`waitpid`, exit `21`), and `/BIN/MUSL.ELF`. M0 dynamic glibc is also green: `hello_glibc` prints `hello, glibc on vibe-os` and exits `11`, while `libc_probe` prints `libc probe ok: noenv 42 pid=4` and exits `13`; `rseq(386)` still returns the tolerated `-ENOSYS`. M1a BusyBox now uses `tools/build_busybox_vibe.sh`, a shell-only/no-network builder over the ignored BusyBox source tree and Zig target tools; `tools/mkrootfs.sh` installs `/BIN/BUSYBOX.ELF` and `/ETC/CATOK.TXT` when those sources exist, with `VIBE_SKIP_BUSYBOX_BUILD=1` available to reuse an existing build. The builder now enables `ash`, `sh`, `echo`, `true`, `ls`, `cat`, `cp`, `grep`, `sleep`, `ps`, `head`, `ln`, `mkdir`, `mv`, `printf`, `pwd`, `rm`, `stat`, `tail`, `test`, `[`, and `wc`; the current ignored artifact is 192260 bytes. Focused smokes now prove `echo`, `true`, `sh -c`, `ls /BIN`, root `ls /`, `cat`, `cp /CP.TXT`, `grep`, scheduler-backed `sleep`, and focused `ps` exit `0` with `linux_last_unimpl_nr=0`; the `ps` proof uses a tiny selector-gated synthetic `/proc` seed and does not claim full procfs. The newly enabled BusyBox applets beyond that proof set are build coverage only until guest-smoked. Full M1 is not complete: interactive prompt, shell-composed pipes/redirection, Ctrl-C/job-control signals, generic procfs/devfs/symlink surfaces, and broader coreutils remain pending. The Linux personality also has a static i386 Linux Doom proof from `/BIN/LDOOM.ELF` with the external shareware WAD, reaching `ldoom frame=1 checksum=983fe686 nonzero=304` and exit `0`.
>
...
- `tools/build_busybox_vibe.sh` / `build/busybox-i386/busybox-vibe` — shell-only BusyBox 1.35.0 static i386 musl builder and ignored `ET_EXEC` artifact (currently 192260 bytes, linked at `0x00e80000`) installed by `tools/mkrootfs.sh` as `/BIN/BUSYBOX.ELF` when present.
...
- `tools/mkrootfs.sh` — rebuilds `build/disk.img` with Linux test binaries injected through `IMAGE_EXTRA_ROOT_ELF_ARGS`; when available, also installs `/BIN/BUSYBOX.ELF` and `/ETC/CATOK.TXT`, with `VIBE_SKIP_BUSYBOX_BUILD=1` for BusyBox reuse.
...
**Status:** Kernel-side startup syscall batch is implemented and proved with `/BIN/STARTUP.ELF` in the current worktree. A real static musl binary is also built with Zig and proved as `/BIN/MUSL.ELF`. Exec staging now uses a compact heap-backed arena instead of fixed argv/env `.bss` buffers, with path length 4096, 64 argv pointers, 64 env pointers, 4096-byte per-string caps, and a shared 32 KiB string pool. `/BIN/XLIMIT.ELF` proves the new path by receiving 10 argv entries, 9 env entries, and long strings crossing the old 64-byte cap. `/BIN/DIR.ELF` starts M1a by proving directory FDs plus Linux `dirent64` output for FAT root and `/BIN`; `/BIN/CWDDIRFD.ELF` now proves `chdir("/BIN")`, `getcwd`, cwd-relative `open`/`openat`/`stat64`, dirfd-relative `openat`/`fstatat64`, and negative relative misses that must not fall back to root; `/BIN/FD.ELF` continues M1a by proving descriptor duplication and fcntl flag state; `/BIN/PIPE.ELF` proves fixed-size pipe buffering plus `pipe2(O_CLOEXEC)`; `/BIN/FORK.ELF` proves a full-copy Linux `fork` seed with parent/child memory divergence and `waitpid` status encoding. `/BIN/BUSYBOX.ELF` now proves real BusyBox applet dispatch through the Linux personality for `echo`, `true`, `sh -c 'echo busybox-ok'`, `ls /BIN`, root `ls /`, `cat`, `cp /CP.TXT`, `grep`, `sleep`, and focused `ps`, all exiting `0` cleanly with `linux_last_unimpl_nr=0`. `sleep` is backed by Linux `nanosleep(162)` -> native tick sleep; `sh` also needed a root-only `getcwd(183)` seed plus i386 `get*id32` aliases. The `ps` proof uses tiny selector-gated synthetic `/proc` process data, so generic procfs remains future work. Linux i386 calls exercised by these focused smokes include: `fork(2)`, `waitpid(7)`, `brk(45)`, `mmap2(192)`, `mprotect(125)`, `munmap(91)`, `writev(146)`, `set_tid_address(258)`, `rt_sigprocmask(175)`, `ioctl(54)` returning `-ENOTTY` for fd 1 `TCGETS`, `getrandom(355)`, `readlink(85)`, `readlinkat(305)`, `wait4(114)` with `rusage=NULL`, seed `open/openat(O_DIRECTORY)`, `getdents64(220)`, `chdir(12)`, `getcwd(183)` for root and one-level FAT cwd, `pipe(42)`, `dup(41)`, `dup2(63)`, `fcntl(55)`, `fcntl64(221)`, `dup3(330)`, `pipe2(331)`, `getpid(20)`, `nanosleep(162)`, `get*id32` aliases, `prctl(172)` as a no-op, and `sched_getaffinity(242)` returning a single-CPU mask. Static glibc is not yet built on this Mac: Zig reports that `x86-linux-gnu` libc requires dynamic linking, and no separate Linux i386 static glibc toolchain is installed.
...
Current M1a BusyBox seed: `tools/build_busybox_vibe.sh` is a shell-only/no-network builder that uses the existing ignored BusyBox source tree plus Zig target tools. It builds BusyBox 1.35.0 as a static i386 musl `ET_EXEC` binary, no PIE, linked at `0x00e80000`, copied to `build/busybox-i386/busybox-vibe`, and installed by `tools/mkrootfs.sh` as `/BIN/BUSYBOX.ELF` when present. The current artifact is 192260 bytes. `tools/mkrootfs.sh` also installs `/ETC/CATOK.TXT` from `tests/linux/catok.txt` when present; set `VIBE_SKIP_BUSYBOX_BUILD=1` to reuse an existing BusyBox artifact. The builder enables `ash`, `sh`, `echo`, `true`, `ls`, `cat`, `cp`, `grep`, `sleep`, `ps`, `head`, `ln`, `mkdir`, `mv`, `printf`, `pwd`, `rm`, `stat`, `tail`, `test`, `[`, and `wc`, with guest proof still limited to the smokes named below. Focused QEMU smokes boot with `LINUX_M1_SMOKE`, exec `/BIN/BUSYBOX.ELF` through the Linux personality, and stage argv as real BusyBox applet invocations:
...
- `busybox sh -c 'echo busybox-ok'`, `busybox ls /BIN`, `busybox ls /`, `busybox cat /ETC/CATOK.TXT`, `busybox cp /ETC/CATOK.TXT /CP.TXT`, `busybox grep ... /ETC/CATOK.TXT`, `busybox sleep 1`, and focused `busybox ps` now exit `0` cleanly; the cp proof also asserts `/CP.TXT` appears in the post-run FAT image with the expected size.

The successful BusyBox applet proofs require/tolerate the existing static-libc startup surface plus `prctl(172)` as a no-op and `sched_getaffinity(242)` returning a single-CPU mask. `ash`/`sh` are compiled in and the non-interactive `sh -c` selector is now clean, but it is not the M1 interactive shell yet. The focused `ps` selector proves a minimal synthetic `/proc` enough for the applet, not the full procfs/devfs/symlink surface. Full M1 remains open until an interactive prompt, shell-composed pipes/redirection, Ctrl-C job/signal behavior, generic procfs/devfs/symlink surfaces, and broader coreutils pass.
