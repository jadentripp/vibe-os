...
>
> The current browser pressure probe packages the current available Debian sid i386 Chromium 148.0.7778.178-1 binary as `/BIN/CHROMIUM.ELF`, matching `chromium-common` ICU data as `/BIN/ICUDTL.DAT`, sid `libc6_2.42-16_i386.deb`, and the 101 recursive real sid i386 `NEEDED` shared libraries under `/LIB` aliases. Latest proof with QEMU `-m 256M -cpu qemu32,+sse,+sse2` records `exec=OK`, `path=/BIN/CHROMIUM.ELF`, `largelf=00000006`, `largebias=08000000/08000000/17CF245C/18000000`, `largedem=00000FFF/00000FFF/00000000/00000002/0F3A4A76/...`, `sock=.../00000000`, `clone=00000001/00000001/.../00000005`, `fork=.../00003125/.../00000000`, `pmmwin=00100000/0FFE0000`, `pmmmap=00050000/0000FF00`, `pmmchk=OK`, no panic, and no shutdown failure. Chromium is now past the Crashpad `setsockopt` and clone ENOMEM blockers; the current boundary is a contained user #UD at `EIP=0x0F3A4A80`, with `faultinsn=0B213000/00000A80/89078BF0/50FF243C/89088B08/51FF2404`, apparently from control flow landing on the trailing `F0` byte of the previous Chromium instruction. This is not a rendered browser UI yet.

> **Chromium update (2026-05-31):** the browser pressure probe now stages the real sid i386 dependency chain plus Chromium ICU data far enough to run browser-owned startup after clone. The kernel gained synthetic Linux library dirs, explicit libc/ld-linux/GLib/GObject/GIO/HarfBuzz-subset aliases, synthetic-dir relative `openat`, recognized optional `rseq(386)`, `-ENOENT` for absent absolute Linux paths such as `/etc/ld.so.cache`, file-backed demand paging for Chromium/ICU, present-PTE `mprotect` updates, SSE/FPU context save/restore, `clock_gettime`, `uname`, `gettid`, a pipe-backed `socketpair` seed, `setsockopt(SOL_SOCKET, SO_PASSCRED)` for that seed, and fork-like `clone(120)` implemented through the full-copy fork path. The latest smoke no longer dies in `ld-linux`, no longer ends on socket-option `-ENOSYS`, and no longer runs out of frames while cloning; it now stops at a contained user-mode invalid-opcode fault in Chromium code, with clean PMM accounting and no kernel panic or shutdown failure.
...
**Architecture:** Add a per-process **personality flag** to the existing process record. The `int 0x80` dispatcher (currently a `cmp eax, SYS_*` chain at `kernel/kernel.asm:22429`) branches to a **separate Linux i386 syscall path** when the flag is `LINUX`; native programs (Doom/Quake) are untouched. A new **Linux ELF32 loader** maps `PT_LOAD` segments and builds a SysV-i386 initial stack (argc/argv/envp + minimal auxv). The current proof kernel keeps a compact `linux_last_unimpl_nr` status field rather than a full trace ring, and the BIOS boot path now reads a 360-sector kernel window (184320 bytes) with the temporary ELF load buffer at `0x72000`; the smoke status window is 64 KiB at `0x40000`, and the PMM frame map lives at `0x50000` so low kernel stacks/page tables and frame accounting stay separated.
...
Current Chromium pressure proof: the current available Debian sid i386 `chromium_148.0.7778.178-1_i386.deb` binary package is packaged as `/BIN/CHROMIUM.ELF` from `/usr/lib/chromium/chromium`. The package SHA-256 is `627276adfbd983e1537403f9f8382db27f100f3944df1f2279ca3cc65b93665e`; the browser ELF SHA-256 is `8a65db2dac7bebd1cf932290139e110fae970efbf6be5a2351188f824fee7c2a`. Matching `chromium-common_148.0.7778.178-1_i386.deb` supplies `/BIN/ICUDTL.DAT`; its package SHA-256 is `26344d8b9cf82e61ad67240f46ccf24cf2b902a47d29e51681c3440842c735b3`, and `icudtl.dat` SHA-256 is `bd8c145abdf3f8383276ce01dfa4ae48709bef9fef1c0711eb7c3fab4f6eb7c2`. The current Chromium smoke uses separate ignored Debian sid i386 `libc6_2.42-16_i386.deb` artifacts for `/LIB/LDLINUX.SO2` and `/LIB/LIBC.SO6`; the package SHA-256 is `4581e89be4256fe0b75c1ba7090477b65d1a4c893ddc0d438c518c6dd8e8e8fb`, ld-linux SHA-256 is `2527ad9fdf19bc90734f17e10922d188dff9095abfcc1d06f97a0c1443f9f508`, and libc SHA-256 is `7361db7f1c6692adedfb8a1cc4956cb0bddef201aef3aa4ba53a28f0ceff8c51`. The latest focused smoke stages the 101 recursive real sid i386 `NEEDED` shared-library assets and reports `exec=OK path=/BIN/CHROMIUM.ELF largelf=00000006/00000003/0000000C/00000004/00000001/00000001/00000000/0FCF245C/0FA71001 largeentry=019E7000 largebias=08000000/08000000/17CF245C/18000000 largemap=00001004/0F3A4000/073A3000/0B213000/00000000/00FDF535 largedem=00000FFF/00000FFF/00000000/00000002/0F3A4A76/0F3A4000/073A3000/00001000`, with Linux personality selected, real sid ld-linux loaded, no kernel panic, no shutdown failure, and clean PMM accounting over `pmmwin=00100000/0FFE0000` plus `pmmmap=00050000/0000FF00`. The proof path pre-maps the main ELF `PT_DYNAMIC` page, reserves the ld.so staging region before sparse page allocation, demand-pages Chromium and ICU file-backed data, handles inherited present-but-supervisor identity mappings inside the browser VMA by replacing them with user page tables, updates present PTE permissions for `mprotect`, reads/writes high physical process page tables through copy aliases, supplies synthetic Linux library dirs, recognizes optional `rseq(386) -> -ENOSYS`, saves/restores SSE/FPU context, provides seed `socketpair`, accepts `setsockopt(SOL_SOCKET, SO_PASSCRED)` on that pipe-backed socketpair, and routes fork-like `clone(SIGCHLD|CLONE_CHILD_SETTID|CLONE_CHILD_CLEARTID)` through the full-copy fork path. Chromium is past the old loader/libc symbol surface, the socket-option `-ENOSYS`, and the clone ENOMEM boundary. Browser UI remains unproved: the current boundary is a contained user #UD at `EIP=0x0F3A4A80`, with `faultsrc=USER`, `faultmode=USER`, and `faultinsn=0B213000/00000A80/89078BF0/50FF243C/89088B08/51FF2404`, apparently from control flow landing on the trailing `F0` byte of the previous Chromium instruction.
...
**Next:** continue M1a from the static BusyBox echo/true proof toward BusyBox `ash`, `ls`, `cat`, and scripted shell behavior; in parallel, harden the M0 dynamic-linker seed around real signal/futex semantics and protection behavior. For the browser pressure path, diagnose why current Chromium/ld.so control flow reaches the middle of a Chromium instruction after clone, then fill the next real Linux surface (`/usr/lib/os-release`, `/proc`, `/dev`, broader sockets, resource packs/locales, graphics/input, process/sandbox).
