...
`dry-run: no files were copied`, and QEMU is not launched. It also shows the
required `phase_hash_*` notes and the short human action notes that later bind
the review checklist to the noVNC session.
...
the required hash note plus the 350-tick duration gate from `after-start` to
`final`.
...
After each successful capture, the guided helper prints the phase's SHA-256 as
`phase_hash_*=...`; those values must match the generated notes, session,
review, checklist, and manifest after download.
...
dumps, raw audio, and any other forbidden proof artifact:
...
`action_note_start=...`, `action_note_fire=...`,
`action_note_move=...`, `action_note_use=...`,
`action_note_mouse=...`, `action_note_menu=...`,
`action_note_final=...`,
...
`no_pixel_upload=yes`, `no_screenshot_upload=yes`,
`no_raw_audio_upload=yes`, `operator_scripted_proof_green=confirmed`,
...
commands plus the action notes and phase hashes copied from the status files.
The checker rebuilds it from the notes and status files so stale checklist text
cannot pass as fresh human evidence. Its `Reviewer runnable checklist` section
is the checklist a reviewer should run after the noVNC session and after
downloading only the allowlisted bundle.
...
already contains the required manual key or mouse action bits. When the
collector's action notes are present, the summary reports how many of the seven
required start/fire/move/use/mouse/menu/final notes were recorded.
...
- The notes are `schema=human-playtest-notes-v2`, include every `phase_hash_*`
  field, include every `action_note_*` field for the human noVNC actions,
  include the per-control `operator_*` confirmation fields, include
  ...
  `slowdown_notes=`, `no_screenshot_upload=yes`, `no_raw_audio_upload=yes`, and
  the local
...
- The generated checklist is `schema=human-playtest-checklist-v1`, names the
  ...
  note, reviewer, remote machine shape, action notes, phase review notes, phase
  hashes, forbidden-artifact exclusions, and local checker commands you used
  after download.
