...
| `ata-storage-stalled` | `atawait=BUSY`, `atawait=DRQ`, or `atawait=READY` before Doom frames, or nonzero `atafail` / `atatmo`; use `ataop`, `atalba`, `atastat`, and `ataerr` for detail | The kernel is stuck in or has failed an ATA PIO wait before Doom produced frames. | `ata_wait_not_busy`, `ata_wait_drq`, `ata_wait_ready`, last LBA selection, and ATA command/status bits. |
...
- `ataop`, `atawait`, `atalba`, `atastat`, `ataerr`, `atafail`, and `atatmo`
  ...
  `atawait=DRQ atastat=00000080` or parks in `atawait=READY` with no Doom frames
  is an ATA PIO wait lane, not a Doom gameplay lane.
