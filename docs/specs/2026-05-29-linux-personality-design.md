...
| **Paging / address spaces** | per-process page dir, CR3 switching (`KERNEL_RELOC_ABI_LIVE_CR3_SWITCH`), PMM cap raised for 256 MiB QEMU browser proofs (`pmmwin=00100000/0FFE0000`, frame map `0x00050000/0x0000FF00`) | Per-process VM exists. Current M1a seed can full-copy fork pages through high-kernel copy aliases, including the Chromium fork-like `clone` proof; true COW remains future work. |
| **Process model** | **fixed 6 slots** (`PROCESS_SLOT_COUNT equ 6`), per-proc brk + heap bitmap + personality/TLS state | Works, but fixed/small — see §9. |
...
| **Filesystem** | FAT with VFS ABI (open/read/write/lseek/stat/fstat/listdir), root+table caches | Files work, and the Linux path now has a seed `open/openat(O_DIRECTORY)` + `getdents64(220)` bridge for FAT root and one-level subdirectories; **no symlinks, no cwd-aware path model, 8.3/LFN, case-insensitive** — see §9. |
...
| **Static BusyBox seed** | BusyBox 1.35.0, static i386 musl, `ET_EXEC`, no PIE, linked at `0x00e80000`; ignored source/output live under `build/busybox-src/busybox-1.35.0` and `build/busybox-i386/busybox-vibe` (112460 bytes), with rootfs injection as `/BIN/BUSYBOX.ELF` when present | `LINUX_M1_BUSYBOX_SMOKE` execs `/BIN/BUSYBOX.ELF` through the Linux personality as `busybox echo busybox-ok`, prints `busybox-ok`, and exits `0`; `LINUX_M1_BUSYBOX_TRUE_SMOKE` proves `busybox true`, also exit `0`. BusyBox `ash` is built in, but `busybox sh -c 'echo busybox-ok'` currently exits `1` with `sh: out of memory`; no fault, panic, shutdown failure, or unexpected `-ENOSYS` is observed in that failed shell attempt. |
...
| **Current Chromium probe** | Current available Debian sid i386 Chromium binary package `chromium_148.0.7778.178-1_i386.deb` was downloaded into ignored `build/chromium-i386/`; package SHA-256 `627276adfbd983e1537403f9f8382db27f100f3944df1f2279ca3cc65b93665e`, browser ELF SHA-256 `8a65db2dac7bebd1cf932290139e110fae970efbf6be5a2351188f824fee7c2a`, size 262608780 bytes. Matching `chromium-common_148.0.7778.178-1_i386.deb` is also staged; package SHA-256 `26344d8b9cf82e61ad67240f46ccf24cf2b902a47d29e51681c3440842c735b3`, with `icudtl.dat` SHA-256 `bd8c145abdf3f8383276ce01dfa4ae48709bef9fef1c0711eb7c3fab4f6eb7c2`. Chromium smoke uses ignored Debian sid i386 `libc6_2.42-16_i386.deb` under `build/chromium-runtime-i386/libc6-sid/` for `/LIB/LDLINUX.SO2` and `/LIB/LIBC.SO6`; package SHA-256 `4581e89be4256fe0b75c1ba7090477b65d1a4c893ddc0d438c518c6dd8e8e8fb`, ld-linux SHA-256 `2527ad9fdf19bc90734f17e10922d188dff9095abfcc1d06f97a0c1443f9f508`, libc SHA-256 `7361db7f1c6692adedfb8a1cc4956cb0bddef201aef3aa4ba53a28f0ceff8c51`, plus the 101 recursive `NEEDED` sonames recorded in ignored `build/chromium-recursive-sonames.txt`. The Chromium ELF is i386 `ET_DYN`/PIE, dynamically linked, interpreter `/lib/ld-linux.so.2`, entry `0x019e7000`, and has large `PT_LOAD` VMAs reaching `0x0fcf245c`. | The FAT image geometry supports a 512 MiB rootfs and packages `/BIN/CHROMIUM.ELF` plus `/BIN/ICUDTL.DAT`. `LINUX_M1_CHROMIUM_SMOKE` now execs that real browser ELF through `PERSONALITY_LINUX`, applies the `0x08000000` ET_DYN load bias, sparse-maps the first page of each `PT_LOAD`, pre-maps the main ELF page containing `PT_DYNAMIC`, reserves the ld.so staging window so interpreter loading does not overwrite sparse main-ELF pages, loads real sid `/LIB/LDLINUX.SO2`, demand-pages Chromium and ICU file-backed pages from FAT, accepts Chromium's pipe-backed `setsockopt(SOL_SOCKET, SO_PASSCRED)`, and accepts its fork-like `clone(SIGCHLD|CLONE_CHILD_SETTID|CLONE_CHILD_CLEARTID)` through the full-copy fork path. The latest local proof used QEMU `-m 256M -cpu qemu32,+sse,+sse2` and recorded `exec=OK path=/BIN/CHROMIUM.ELF`, `largelf=00000006/00000003/0000000C/00000004/00000001/00000001/00000000/0FCF245C/0FA71001`, `largebias=08000000/08000000/17CF245C/18000000`, `largemap=00001004/0F3A4000/073A3000/0B213000/00000000/00FDF535`, `largedem=00000FFF/00000FFF/00000000/00000002/0F3A4A76/0F3A4000/073A3000/00001000`, `sock=.../00000000`, `clone=00000001/00000001/.../00000005`, `fork=00000001/.../00003125/.../00000000`, `pmmwin=00100000/0FFE0000`, `pmmmap=00050000/0000FF00`, `pmmchk=OK`, `faultsrc=USER`, `faultmode=USER`, `faultinsn=0B213000/00000A80/89078BF0/50FF243C/89088B08/51FF2404`, `panic=NONE`, and `shutdown=NONE`. The current boundary is a contained user #UD at `EIP=0x0F3A4A80`, not an unexpected final `-ENOSYS`; decoded bytes show execution began on the trailing `F0` byte of the previous instruction, producing invalid `LOCK MOV`. This is current Chromium, not the old Chromium compatibility path, not headless shell, and not a native shim; it still does **not** render a browser UI. |
| **Conventions** | assembly-first, C modest, **no Python** (`AGENTS.md`) | Personality is kernel work → assembly-first. |

**Current Chromium update (2026-05-31):** the current Debian sid i386 Chromium proof has advanced past the earlier missing-library, `stderr@GLIBC_2.0`, `setsockopt`, and fork/clone ENOMEM boundaries. The ignored rootfs stages the real recursive `NEEDED` dependency chain (101 sonames) plus matching `chromium-common` ICU data; no shim libraries were added. Kernel-side Linux compatibility now includes synthetic `/lib`, `/usr/lib`, and i386 multiarch library dirs, explicit 8.3 aliases for libc/ld-linux/GLib/GObject/GIO/HarfBuzz subset, relative `openat` support for synthetic library dir FDs, `rseq(386)` as a recognized optional `-ENOSYS`, Linux absolute missing paths returning `-ENOENT` rather than native `-EINVAL`, file-backed `mmap2`/demand paging for the browser and ICU files, present-PTE permission updates for `mprotect`, SSE/FPU context switching via `fxsave`/`fxrstor`, a seed pipe-backed `socketpair`, `setsockopt(SOL_SOCKET, SO_PASSCRED)` for that pipe-backed socketpair, and a fork-like `clone(120)` path implemented as full-copy fork rather than COW. The latest Chromium smoke reaches browser-owned startup after clone; status remains `exec=OK path=/BIN/CHROMIUM.ELF`, Linux personality selected, `pmmwin=00100000/0FFE0000`, `pmmmap=00050000/0000FF00`, `pmmchk=OK`, `panic=NONE`, and `shutdown=NONE`. The current blocker is a contained user-mode #UD at `EIP=0x0F3A4A80` with `faultinsn=0B213000/00000A80/89078BF0/50FF243C/89088B08/51FF2404`, apparently from control flow landing on the trailing `F0` byte of the previous Chromium instruction. Next work is diagnosing that bad userland control-flow target, then broader sockets, `/proc`/`/dev`, `/usr/lib/os-release` and other synthetic files, resource-pack/locales packaging, process/sandbox, graphics/input, and many more syscalls.
...
**Current BusyBox proof:** the first static BusyBox slice now boots as a real Linux-personality guest, not a native shim. The rootfs hook installs `build/busybox-i386/busybox-vibe` as `/BIN/BUSYBOX.ELF`; the focused smoke selectors exec that path and stage Linux argv directly. `busybox echo busybox-ok` prints the marker and exits `0`, and `busybox true` exits `0`. Status for both shows `exec=OK`, `path=/BIN/BUSYBOX.ELF`, Linux personality selected, exit code `0`, `last_error=0`, `linux_last_unimpl_nr=0`, no page fault, no panic, and no shutdown failure. The required/tolerated syscall surface for this proof includes the M-1 static-libc startup set (`brk(45)`, `mmap2(192)`, `mprotect(125)`, `munmap(91)`, `set_tid_address(258)`, `set_thread_area(243)`, `rt_sigprocmask(175)`, `ioctl(54)` returning `-ENOTTY`, `getrandom(355)`, `readlink(85)`, `readlinkat(305)`, `write(4)`, `writev(146)`, `exit(1)`, `exit_group(252)`) plus `prctl(172)` as a no-op and `sched_getaffinity(242)` returning a single-CPU mask. BusyBox `ash` is intentionally not counted as done yet: `busybox sh -c 'echo busybox-ok'` currently reaches userland and exits cleanly from the kernel's point of view, but prints `sh: out of memory` and returns `1`.
...
**Current Chromium proof boundary:** the first "real browser" probe uses the current available Debian sid i386 Chromium binary package (`chromium_148.0.7778.178-1_i386.deb`) and packages `/usr/lib/chromium/chromium` as `/BIN/CHROMIUM.ELF` in the FAT image, with matching `chromium-common` ICU data installed as `/BIN/ICUDTL.DAT`. The image builder and stage2 FAT geometry were lifted to a 512 MiB image with 16-sector FAT clusters so the 262608780-byte ELF can exist in the guest rootfs. The smoke selector `LINUX_M1_CHROMIUM_SMOKE` execs `/BIN/CHROMIUM.ELF` through the Linux personality with Debian sid i386 `libc6_2.42-16_i386.deb` providing `/LIB/LDLINUX.SO2` and `/LIB/LIBC.SO6`, plus 101 recursive real sid i386 `NEEDED` shared libraries under `/LIB` aliases. Current status after a local TCG settle with QEMU `-m 256M -cpu qemu32,+sse,+sse2`: `exec=OK`, `path=/BIN/CHROMIUM.ELF`, `largelf=00000006/00000003/0000000C/00000004/00000001/00000001/00000000/0FCF245C/0FA71001`, `largeentry=019E7000`, `largebias=08000000/08000000/17CF245C/18000000`, `largemap=00001004/0F3A4000/073A3000/0B213000/00000000/00FDF535`, and `largedem=00000FFF/00000FFF/00000000/00000002/0F3A4A76/0F3A4000/073A3000/00001000`. Status `6` is `LARGE_ELF_STATUS_SPARSE_MAPPED`: the loader recognized `ET_DYN`, 12 program headers, 4 `PT_LOAD`s, and matching `PT_INTERP`; sparse-mapped the first page of each `PT_LOAD`; pre-mapped the `PT_DYNAMIC` page; reserved the ld.so staging window so loading the interpreter cannot overwrite sparse main-ELF pages; loaded real sid `/LIB/LDLINUX.SO2`; then demand-paged Chromium and ICU file-backed pages from FAT while ld-linux and Chromium executed. The demand path also handles inherited present-but-supervisor PDE/PTE state from copied kernel identity mappings inside the large browser VMA by replacing it with user page tables, updates present PTE permissions for `mprotect`, and reads/writes high physical process page tables through copy aliases. Chromium then passes the previous Crashpad `setsockopt` blocker with `setsockopt(SOL_SOCKET, SO_PASSCRED)` on the pipe-backed socketpair, and passes the previous fork/clone ENOMEM blocker by routing fork-like `clone(SIGCHLD|CLONE_CHILD_SETTID|CLONE_CHILD_CLEARTID)` through the full-copy fork path (`fork=.../00003125/.../00000000`). PMM accounting remains clean (`pmmwin=00100000/0FFE0000`, `pmmmap=00050000/0000FF00`, `pmmchk=OK`), with `panic=NONE` and `shutdown=NONE`. The current boundary is a contained user-mode invalid-opcode fault: `fault=00000006/.../0F3A4A80/.../000000AF`, `faultsrc=USER`, `faultmode=USER`, and `faultinsn=0B213000/00000A80/89078BF0/50FF243C/89088B08/51FF2404`. Decoding the captured bytes shows Chromium began executing at the trailing `F0` byte of the previous instruction, yielding invalid `LOCK MOV`; the next browser slice is to explain that bad userland control-flow target, then broaden sockets, `/proc`, `/dev`, `/usr/lib/os-release`, resource-pack/locales packaging, graphics/input, process/sandbox, and syscall work.
...
11. **BusyBox** applets: current proof covers `busybox echo busybox-ok` and `busybox true` from `/BIN/BUSYBOX.ELF` under the Linux personality; next applet targets are `ls`, `cat`, `cp`, `grep`, and `ash`/interactive shell once heap, process, filesystem, tty, and signal gaps are closed.
...
13. **Current Chromium pressure probe** from `/BIN/CHROMIUM.ELF`: current proof packages the real Debian i386 Chromium browser ELF plus matching `/BIN/ICUDTL.DAT`, recognizes `ET_DYN`, 12 program headers, 4 `PT_LOAD`s, and `/lib/ld-linux.so.2`, applies a documented `0x08000000` ET_DYN load bias, sparse-maps the first page of each load, pre-maps the main ELF `PT_DYNAMIC` page, runs real `/LIB/LDLINUX.SO2`, demand-pages Chromium/ICU file-backed data, accepts Crashpad's `setsockopt(SOL_SOCKET, SO_PASSCRED)`, accepts fork-like `clone` through full-copy fork, and stops at a contained user #UD after browser-owned startup. This is not an M1 acceptance gate; it is now the first browser-sized loader/address-space/userland-startup/process proof and the current blocker is a bad userland control-flow target, not whole-file size, missing shared-library packaging, the old `stderr@GLIBC_2.0` loader boundary, socket-option `-ENOSYS`, or fork ENOMEM.
...
- ⚠️ **HEADLINE: official Chrome still wants 64-bit, but current i386 Chromium is a useful pressure probe.** vibe-os is `bits 32`. Official Google Chrome for Linux is not the same target as Debian's i386 Chromium package; the former still drives a long-mode/x86-64 milestone, while the latter currently gives this 32-bit Linux personality a real browser-sized ELF to push against. The current-Chromium proof gets past the old whole-file size reject, dynamic-table/PTE hazards, missing-library loop, `stderr@GLIBC_2.0` loader boundary, Crashpad `setsockopt` `-ENOSYS`, and fork-like clone ENOMEM: it execs the browser ELF, loads real ld-linux, sparse-maps the huge PIE, protects the main ELF dynamic page from interpreter staging overwrite, demand-pages Chromium and ICU file data, opens real Linux-layout library paths, handles pipe-backed `setsockopt(SO_PASSCRED)`, full-copy clones a Chromium child, records a contained user-mode #UD at a browser address, and keeps PMM accounting clean. It still does not render a browser UI; the observed blocker is Chromium/loader control flow landing mid-instruction in userland. **Recommendation:** keep M-1→M1 on 32-bit to build the personality machinery, use current Debian i386 Chromium as a pressure probe, and track a long-mode port as its own major milestone for official Chrome and modern browser endgame work.
