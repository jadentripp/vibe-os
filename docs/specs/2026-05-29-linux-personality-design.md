...
**Latest Chromium smoke note:** parent evidence after the `0x77000` status-capture move supersedes older Chromium notes: current artifacts reach `exec=OK`/interpreter/sparse-demand paging for the real browser ELF, then remain at the early demand-page boundary around `/etc/ld.so.cache -> -ENOENT` and `0x08010D76`. The older `setsockopt`/clone/#UD details remain useful historical context, not the current boundary until revalidated.
...
**Current Chromium proof boundary:** the first "real browser" probe uses the current available Debian sid i386 Chromium binary package (`chromium_148.0.7778.178-1_i386.deb`) and packages `/usr/lib/chromium/chromium` as `/BIN/CHROMIUM.ELF` in the FAT image, with matching `chromium-common` ICU data installed as `/BIN/ICUDTL.DAT`. The image builder and stage2 FAT geometry were lifted to a 512 MiB image with 16-sector FAT clusters so the 262608780-byte ELF can exist in the guest rootfs. The smoke selector `LINUX_M1_CHROMIUM_SMOKE` execs `/BIN/CHROMIUM.ELF` through the Linux personality with Debian sid i386 `libc6_2.42-16_i386.deb` providing `/LIB/LDLINUX.SO2` and `/LIB/LIBC.SO6`, plus 101 recursive real sid i386 `NEEDED` shared libraries under `/LIB` aliases. Current relocated-status evidence records `exec=OK`, `path=/BIN/CHROMIUM.ELF`, `interp=00000001/00000001`, `largelf=00000006/.../0FCF245C/0FA71001`, `largeentry=019E7000`, `largebias=08000000/08000000/17CF245C/18000000`, sparse mapping, and demand paging beginning for the real browser ELF with no kernel panic or shutdown failure. The fresh boundary is early ld.so/Chromium demand paging around `/etc/ld.so.cache -> -ENOENT` and a user page fault at `0x08010D76`. The earlier Crashpad `setsockopt`, fork-like `clone`, and contained #UD evidence must be re-reached after the status-buffer relocation before being treated as current.

Current parent correction: the newest Chromium evidence after the status-buffer move exits `127` around `openat`/`access` probing for `/usr/lib/.../libnspr4.so`. Keep the `exec=OK`, interpreter, sparse mapping, and demand-paging-begins claims; demote the older `0x08010D76`, socket/clone, and #UD details to historical notes until revalidated.
...
13. **Current Chromium pressure probe** from `/BIN/CHROMIUM.ELF`: current proof packages the real Debian i386 Chromium browser ELF plus matching `/BIN/ICUDTL.DAT`, recognizes `ET_DYN`, 12 program headers, 4 `PT_LOAD`s, and `/lib/ld-linux.so.2`, applies a documented `0x08000000` ET_DYN load bias, sparse-maps the first page of each load, pre-maps the main ELF `PT_DYNAMIC` page, runs real `/LIB/LDLINUX.SO2`, and begins demand-paging browser file-backed data. This is not an M1 acceptance gate; it is a browser-sized loader/address-space/userland-startup pressure proof. The current relocated-status blocker is early ld.so/Chromium demand paging around `/etc/ld.so.cache -> -ENOENT` and `0x08010D76`; the older socket-option, fork-like clone, and #UD observations need fresh revalidation.
