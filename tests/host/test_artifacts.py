...
class SourceContractTests(unittest.TestCase):
    def test_local_vm_targets_are_opt_in(self):
        ...
        self.assertIn("make_wad_image.WRITABLE_SAVE_NAMES[slot]", real_wad_workflow)
        self.assertIn('data = f"VIBE_SAVE_{slot}\\n".encode("ascii")', real_wad_workflow)
        self.assertIn("marker_slot = (slot + 1) % len(make_wad_image.WRITABLE_SAVE_NAMES)", real_wad_workflow)
        self.assertIn('data = f"VIBE_LOAD_{slot}\\n".encode("ascii")', real_wad_workflow)
        self.assertIn("make_wad_image.WRITABLE_DEFAULT_NAME", real_wad_workflow)
        self.assertIn('data = b\'chatmacro0\\t\\t"VIBE_DEFAULT"\\n\'', real_wad_workflow)
