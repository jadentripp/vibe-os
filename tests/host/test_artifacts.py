...
class SourceContractTests(unittest.TestCase):
    def test_local_vm_targets_are_opt_in(self):
        ...
        self.assertIn('write_marker SAVE_REQUEST_NAME "$slot_marker_payload"', real_wad_workflow)
        ...
        self.assertIn('name = getattr(make_wad_image, os.environ["PERSISTENCE_MARKER_NAME"])', real_wad_workflow)
        self.assertIn('data = os.environ["PERSISTENCE_MARKER_PAYLOAD"].encode("ascii")', real_wad_workflow)
        self.assertNotIn('data = f"VIBE_SAVE_{slot}\\n".encode("ascii")', real_wad_workflow)
        self.assertNotIn('data = f"VIBE_LOAD_{slot}\\n".encode("ascii")', real_wad_workflow)
        self.assertNotIn("read_save_slot_marker_request(", platform)
        self.assertNotIn('default_config_file_contains_marker("VIBE_DEFAULT")', platform)
        self.assertIn("SAVE_REQUEST_NAME = b\"SAVEREQ CHK\"", image_tool)
        self.assertIn("LOAD_REQUEST_NAME = b\"LOADREQ CHK\"", image_tool)
        self.assertIn('write_marker LOAD_REQUEST_NAME "$slot_marker_payload"', real_wad_workflow)
        ...
        self.assertIn('cp "$write_status" build/status.persistence-write.txt', real_wad_workflow)
    ...
    def test_doom_autostarts_e1m1_and_reports_gameplay_state(self):
        ...
        self.assertIn("load_checkpoint_post_tic_pending", platform)
        self.assertIn("static int load_checkpoint_post_load_ready(void)", platform)
        self.assertIn("&& gameaction == ga_nothing", platform)
        self.assertIn("leveltime > load_checkpoint_post_tic_leveltime", platform)
        self.assertIn("gametic != load_checkpoint_post_tic_gametic", platform)
    ...
    def test_kernel_has_dynamic_fat16_writable_file_path(self):
        ...
        self.assertIn("mov eax, [persistence_marker_sizes + eax * 4]", stat_path)
        self.assertLess(
            stat_path.index("call fat_open_name_marker_index"),
            stat_path.index("mov edi, fat_open_name_buffer", stat_path.index("call fat_open_name_marker_index")),
        )
    ...
    def test_doom_port_and_probe_have_indexed_frame_present_syscall(self):
        ...
        self.assertIn("pmemsave 0x9d000 8192", smoke_runner)
    ...
    def test_doom_mouse_events_flow_through_ps2_aux_and_syscall(self):
        ...
        header = (ROOT / "doom_port" / "include" / "vibe_os.h").read_text()
        makefile = (ROOT / "Makefile").read_text()
        mouse_doc = (ROOT / "docs" / "doom-libc-runtime.md").read_text()
        for source in (
