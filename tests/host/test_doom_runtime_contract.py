...
class DoomRuntimeContractTests(unittest.TestCase):
    def test_port_layer_owns_doom_syscall_contract(self):
        ...
        self.assertIn("#define F_GETFD 1", fcntl)
        self.assertIn("#define F_SETFD 2", fcntl)
        self.assertIn("#define FD_CLOEXEC 1", fcntl)
        self.assertIn("int fcntl(int fd, int cmd, ...);", fcntl)
        self.assertIn("ssize_t pread(int fd, void* buffer, size_t count, off_t offset);", unistd)
        self.assertIn("ssize_t pwrite(int fd, const void* buffer, size_t count, off_t offset);", unistd)
        self.assertIn("int vibe_file_read_at(", header)
        self.assertIn("static int validate_open_flags", libc)
        self.assertIn("static ssize_t positioned_io(", libc)
        self.assertIn("int vibe_file_read_at(", libc)
        self.assertIn("O_CLOEXEC", libc)
        self.assertIn("int fcntl(int fd, int cmd, ...)", libc)
        self.assertIn("vibe_syscall3(VIBE_SYS_FCNTL", libc)
    ...
    def test_save_ticcmd_wrapper_preserves_original_doom_source(self):
        ...
        self.assertIn('"PERSIST.CHK"', platform)
        self.assertIn('"SAVEREQ.CHK"', platform)
        ...
        self.assertIn("&& !save_checkpoint_promoted", ticker)
        ...
        self.assertIn("save_checkpoint_promoted = 1;", ticker)
        self.assertIn("if (save_checkpoint_promoted", ticker)
        ...
        self.assertIn("for (slot = 0; slot < BACKUPTICS; ++slot)", platform)
        self.assertIn("netcmds[consoleplayer][slot].buttons = 0;", platform)
