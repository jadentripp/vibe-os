...
- `tools/check_vm_status_proof.py` requires the matching cloud status to expose
  `vmmhfree`, `argvsrc=2`, and `pkind`/`peip`/`pcr3`/`pkstk` evidence before a
  VM/process artifact can be accepted.
...
- `tools/check_vm_status_proof.py` validates cloud status artifacts for the VM
  ...
  Doom exec must report `argvsrc=2`, and `pkind`/`peip`/`pcr3`/`pkstk` must
  show timer IRQ switching between Doom and the preempt probe.
