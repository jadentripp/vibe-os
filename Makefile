...
smoke: vm-consent check-tools $(IMAGE)
	...
	@BUILD_DIR="$(BUILD_DIR)" \
		SMOKE_SKIP_ASSERTIONS="$(SMOKE_SKIP_ASSERTIONS)" \
		SMOKE_CAPTURE_GFX="$(SMOKE_CAPTURE_GFX)" \
		SMOKE_EXPECT_PROBE_GFX="$(SMOKE_EXPECT_PROBE_GFX)" \
		SMOKE_REJECT_DOOMLOG="$(SMOKE_REJECT_DOOMLOG)" \
		SMOKE_REQUIRE_DOOM_PRESENT="$(SMOKE_REQUIRE_DOOM_PRESENT)" \
		SMOKE_REQUIRE_DOOM_GAMEPLAY="$(SMOKE_REQUIRE_DOOM_GAMEPLAY)" \
		SMOKE_REQUIRE_REAL_WAD_PROOF="$(SMOKE_REQUIRE_REAL_WAD_PROOF)" \
		SMOKE_REQUIRE_HUMAN_PLAYABILITY_PROOF="$(SMOKE_REQUIRE_HUMAN_PLAYABILITY_PROOF)" \
		SMOKE_REQUIRE_AUDIO_CONTINUITY="$(SMOKE_REQUIRE_AUDIO_CONTINUITY)" \
		SMOKE_SENDKEYS="$(SMOKE_SENDKEYS)" \
		SMOKE_REQUIRE_KEY_EVENT="$(SMOKE_REQUIRE_KEY_EVENT)" \
