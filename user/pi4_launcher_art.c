...
#define PI4_LAUNCHER_APP_RECORD_COUNT PI4_VIBE_APP_RECORD_CAPACITY
...
static void fit_source_in_icon(
    u32 width,
    u32 height,
    u32* target_x,
    u32* target_y,
    ...
    u32* target_h)
{
    ...
    *target_h = 0;
    ...
    if (width >= height) {
        *target_w = PI4_LAUNCHER_ICON_SIZE;
        *target_h = (height * PI4_LAUNCHER_ICON_SIZE + (width >> 1)) / width;
        if (!*target_h)
            *target_h = 1u;
        *target_y = (PI4_LAUNCHER_ICON_SIZE - *target_h) >> 1;
    } else {
        *target_h = PI4_LAUNCHER_ICON_SIZE;
        *target_w = (width * PI4_LAUNCHER_ICON_SIZE + (height >> 1)) / height;
        if (!*target_w)
            *target_w = 1u;
        *target_x = (PI4_LAUNCHER_ICON_SIZE - *target_w) >> 1;
    }
}
...
static int decode_doom_titlepic_icon(u8* icon, const u8* pixels, u32 pixel_size, const u8* palette)
{
    ...
    for (u32 y = 0; y < target_h; y++) {
        u32 source_y = (y * PI4_LAUNCHER_DOOM_TITLEPIC_HEIGHT) / target_h;
        for (u32 x = 0; x < target_w; x++) {
            u32 source_x = (x * PI4_LAUNCHER_DOOM_TITLEPIC_WIDTH) / target_w;
            u32 source_offset = source_y * PI4_LAUNCHER_DOOM_TITLEPIC_WIDTH + source_x;
            u32 target_offset = (target_y + y) * PI4_LAUNCHER_ICON_SIZE + target_x + x;
            icon[target_offset] = palette_to_launcher_color(
                palette,
...
static int decode_doom_patch_icon(u8* icon, const u8* patch, u32 patch_size, const u8* palette)
{
    ...
    u32 target_h;
    clear_icon(icon);
    fit_source_in_icon(width, height, &target_x, &target_y, &target_w, &target_h);
    if (!target_w || !target_h)
        return 0;
    ...
    for (u32 y = 0; y < target_h; y++) {
        ...
        for (u32 x = 0; x < target_w; x++) {
            ...
            if (doom_patch_sample(patch, patch_size, width, source_x, source_y, &color_index)) {
                u32 target_offset = (target_y + y) * PI4_LAUNCHER_ICON_SIZE + target_x + x;
                icon[target_offset] = palette_to_launcher_color(palette, color_index);
            }
        }
    }
...
static int load_wad_icon(u8* icon, const char* wad_path, const char* icon_spec)
{
    ...
    if (!icon || !wad_path || !icon_spec || !str_prefix(icon_spec, "wad:"))
        return 0;
...
static int load_pak_icon(u8* icon, const char* pak_path, const char* icon_spec)
{
    const char* icon_name;
    u8 header[12];
    u32 icon_offset = 0;
    ...
    if (!icon || !pak_path || !icon_spec || !str_prefix(icon_spec, "pak:"))
        return 0;
    icon_name = icon_spec + 4;
    if (!icon_name[0])
        return 0;
...
static int load_manifest_icon(u8* icon, const char* asset_path, const char* icon_spec)
{
    if (!icon_spec)
        return 0;
    if (str_prefix(icon_spec, "wad:"))
        return load_wad_icon(icon, asset_path, icon_spec);
    if (str_prefix(icon_spec, "pak:"))
        return load_pak_icon(icon, asset_path, icon_spec);
    return 0;
}
